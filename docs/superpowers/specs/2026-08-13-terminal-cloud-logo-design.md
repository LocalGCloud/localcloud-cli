...
The approved mark is a symmetric, empty cloud outline made from single-column Unicode box-drawing characters. It is exactly 20 columns by 5 rows:
...
- centering the 20-column mark in wide, stacked, and compact panels;
...
Every row must retain a visible width of exactly 20 terminal columns. The artwork uses only spaces and single-column box-drawing characters, so the current character-width and centering logic remains valid.
...
- all five rows have visible width 20;
- true-color animation changes ANSI color without changing glyphs;
- wide, stacked, compact, and minimal panels remain aligned;
- the wide startup panel shows the curved outline centered in its existing 30-column cell.
