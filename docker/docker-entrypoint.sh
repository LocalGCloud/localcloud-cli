...
# Verify: PostgreSQL requires the data directory to be owned by the user
# that runs the postgres process (localcloud, UID 1001). The chown above
# silently fails on systems where bind mounts don't support chown (macOS,
# NFS, certain CSI drivers). Catch it early with a clear diagnostic.
DATA_UID=$(stat -c '%u' /var/lib/localcloud/pgdata 2>/dev/null || echo "0")
if [ "$DATA_UID" != "$RUN_UID" ]; then
	DATA_OWNER=$(stat -c '%U:%G' /var/lib/localcloud/pgdata 2>/dev/null || echo "unknown")
	echo "" >&2
	echo "ERROR: PostgreSQL data directory has wrong ownership." >&2
	echo "  Directory : /var/lib/localcloud/pgdata" >&2
	echo "  Owner     : $DATA_OWNER (needs UID $RUN_UID, got UID $DATA_UID)" >&2
	echo "  Runtime   : running as user '$RUN_USER' (UID $RUN_UID)" >&2
	echo "" >&2
	echo "This happens when the bind mount filesystem does not support chown" >&2
	echo "(common on macOS Docker Desktop, NFS mounts, or certain volume drivers)." >&2
	echo "" >&2
	echo "Workarounds (pick one):" >&2
	echo "  1. Use a Docker named volume instead of a bind mount:" >&2
	echo "     docker run -v localcloud-data:/var/lib/localcloud ..." >&2
	echo "" >&2
	echo "  2. Fix ownership on the host before starting:" >&2
	echo "     sudo chown -R 1001:1001 ~/.localcloud/data/pgdata" >&2
	echo "" >&2
	echo "  3. Override the runtime user to match the bind mount owner:" >&2
	echo "     docker run --user \$(stat -c '%u:%g' ~/.localcloud/data/pgdata) ..." >&2
	echo "" >&2
	exit 1
fi
...
# Logs are disposable runtime state. Keep them on the container filesystem so
# the persistent data mount contains only state that needs to be backed up.
# Remove the legacy data/logs directory left by older images during upgrades.
if [ -e /var/lib/localcloud/logs ] || [ -L /var/lib/localcloud/logs ]; then
	echo "Removing legacy logs from persistent data directory..."
	rm -rf /var/lib/localcloud/logs
fi
...
if [ -n "${LOCALCLOUD_SERVICES}" ]; then
	...
	export LOCALCLOUD_ENABLE_CLOUDRESOURCEMANAGER="false"
	export LOCALCLOUD_ENABLE_CLOUDBILLING="false"
	export LOCALCLOUD_ENABLE_SERVICEUSAGE="false"
	...
	for service in "${SERVICES[@]}"; do
		...
		cloudresourcemanager) export LOCALCLOUD_ENABLE_CLOUDRESOURCEMANAGER="true" ;;
		cloudbilling) export LOCALCLOUD_ENABLE_CLOUDBILLING="true" ;;
		serviceusage) export LOCALCLOUD_ENABLE_SERVICEUSAGE="true" ;;
		*)
