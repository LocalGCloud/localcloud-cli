...
function sparklinePoints(data, width = 80, height = 24) {
...
export default function Dashboard(props) {
 ...
 if (health?.cpu?.process_load != null) {
 setCpuHistory(prev => {
 if (prev.length > 0 && prev[prev.length - 1] === health.cpu.process_load) return prev;
 const next = [...prev, health.cpu.process_load];
 return next.length > MAX_HISTORY ? next.slice(-MAX_HISTORY) : next;
 });
 }
 if (health?.memory?.heap_usage_pct != null) {
 setMemHistory(prev => {
 if (prev.length > 0 && prev[prev.length - 1] === health.memory.heap_usage_pct) return prev;
 const next = [...prev, health.memory.heap_usage_pct];
 return next.length > MAX_HISTORY ? next.slice(-MAX_HISTORY) : next;
 });
 }
 });
 ...
 <div class="stat-card-value" >
 {props.healthData()?.cpu?.process_load != null ? `${props.healthData()?.cpu?.process_load}%` : '--'}
 </div>
 <Show when={cpuHistory().length >= 2}>
 <svg width="80" height="24" class="sparkline">
 <polyline fill="none" stroke="var(--primary)" stroke-width="1.5" points={sparklinePoints(cpuHistory(), 80, 24)} />
 </svg>
 </Show>
 </div>
 <div class="stat-card">
 ...
 <div class="stat-card-value" >
 {props.healthData()?.memory?.heap_usage_pct != null ? `${props.healthData()?.memory?.heap_usage_pct}%` : '--'}
 </div>
 <Show when={memHistory().length >= 2}>
 <svg width="80" height="24" class="sparkline">
 <polyline fill="none" stroke="var(--primary)" stroke-width="1.5" points={sparklinePoints(memHistory(), 80, 24)} />
 </svg>
 </Show>
 </div>
 </div>
